...
SQLite v4 audit events record successful local project and candidate state
changes. They do not automatically retain rejected requests, collector
warnings, OS logs, or authenticated actor identity, and an administrator able
to replace the database is outside the append-only trigger boundary.
