...
WINDOWS_ALPHA_EVIDENCE = REPOSITORY_ROOT / "reports" / "PHASE_22_WINDOWS_ALPHA_LIVE_EVIDENCE.json"
...
def test_committed_windows_alpha_evidence_is_complete_and_path_free() -> None:
    raw = WINDOWS_ALPHA_EVIDENCE.read_bytes()
    document = json.loads(raw)
    verification_id = document.pop("verification_id")
    ...
    assert document["report_format"] == "forgegate.windows-alpha-plugin-chain-verification.v1"
    assert document["production_broker_verified"] is True
    assert document["forgegate_installation"] == "CLEAN_WHEEL"
    assert document["advertised_isolation_tier"] == "SANDBOXED"
    assert document["evidence_level"] == "LOCAL_HOST_TEST"
