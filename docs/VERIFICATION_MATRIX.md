...
| Dashboard activation recovery | production-TypeScript controlled DOM/HTTP/clock regressions plus real local-browser activation | PASS — 13 host cases cover origin/port, network/500 retry, 429 wait, expiry, and late responses; port 8131 activated through the companion CLI in the in-app browser; not a Chrome/Edge or assistive retest |
| Project Audit workspace | 12 frontend host cases, 5 BFF cases, isolated real-browser fixture | PASS — operator/project scope, exact candidate filter, 25 + 3 pagination, full fingerprints, missing-actor boundary, refresh/back, keyboard details, stale response isolation, 422 malformed query, and no release-state writes; not a security-log or compliance certification |
...
| Physical MSP430 unplug/replug page transition | owner-assisted manual interaction | BOUNDED PASS — owner observed the live transition; recovered status retained `reconnects=1`, one sequence gap, and one protocol error, then sequence 138→149 and valid frames 3028→3039 advanced over 10.5 seconds without new errors; no automated transition timestamp or long-duration claim |
| MSP430 fault-code presentation | versioned adapter tests + authenticated COM4 page | BOUNDED PASS — `0015` decoded to DS18B20 missing, NTC unavailable/range, and INA219 communication; unknown bits are retained without invented meaning; device reports are not ForgeGate diagnoses or release evidence |
