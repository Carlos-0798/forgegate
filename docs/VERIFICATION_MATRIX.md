...
| Connected MSP430 Windows USB/UART presence | user-authorized device enumeration plus non-writing COM4 open/close, sustained-open, and later re-enumeration | BOUNDED PASS / FOLLOW-UP ABSENT — 10/10 opens and a 10-second sustained connection passed, but COM4/COM5 later reported problem 45 and `Present=false`; no long-duration stability, parsed telemetry, firmware/debug action, protocol/measurement proof, or ForgeGate collector execution |
