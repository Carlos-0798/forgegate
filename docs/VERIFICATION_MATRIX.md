...
| Dashboard BFF OpenAPI contract | separate build-time export + committed byte and installed-wheel comparison | PASS — twelve `/app/api/` operations with explicit operation IDs; not served as interactive docs |
...
| Physical MSP430 unplug/replug page transition | owner-assisted manual interaction | NOT RUN — deterministic absent/stale/error/recovery states pass; a physical transition requires unplugging and reconnecting the board while the page remains open |
...
| Coverage gate precision | two-decimal branch-aware threshold and full suite | PASS — actual 95.32% exceeds exact 95.00% threshold |
