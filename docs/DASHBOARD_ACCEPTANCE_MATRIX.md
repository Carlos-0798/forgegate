...
| Authoritative UI reload | PASS after correction | Browser interaction checks found the table could retain the prior state while detail reloaded. The completion action now reloads the entire candidate workspace; the table and detail both showed PASS revision 4. |
