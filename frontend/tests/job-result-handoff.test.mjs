...
const job={schema_version:'forgegate.collection-job.v2',job_id:`job-${'a'.repeat(32)}`,candidate_id:candidate.candidate_id,project_id:'sample-api',revision:4,state:'SUCCEEDED',request_fingerprint:fp('1'),result_fingerprint:fp('2'),candidate_fingerprint:fp('3'),authority:'AUTHENTICATED_DASHBOARD',source_bytes:'released_logically',updated_at:'2026-09-06T20:00:00Z',created_at:'2026-09-06T19:00:00Z',lease_expires_at:null,error_code:null,execution_owner_id:`executor-${'b'.repeat(32)}`,lease_renewal_count:2};
...
test('canonical download freezes identities, verifies bytes and changes no state',async()=>{
  ...
  assert.deepEqual(body,{expected_job_revision:4,expected_result_fingerprint:job.result_fingerprint,expected_assembly_id:assembly.assembly_id});
...
test('job binding fetches current candidate then submits one frozen identity set',async()=>{
  ...
  assert.equal(body.expected_job_revision,4);assert.equal(body.expected_result_fingerprint,job.result_fingerprint);assert.equal(body.expected_assembly_id,assembly.assembly_id);assert.equal(body.expected_candidate_revision,1);assert.equal(body.expected_candidate_fingerprint,job.candidate_fingerprint);
